...
- **自社 → ドライバー個人**の請求書（車両弁済・備品代など）は `parties = {fromParty:"ace_creation", toParty:"drv-<id>"}` で表す。方向判定上は outgoing（自社が請求）。法人アドレス帳を経由しないので `counterparty_invoice_address_id` は null、`driver_id` も紐づけない（紐づけると `/api/me/invoices` が driver_id だけで引くため、本人の報酬画面＝受領請求書＋承認フローに混ざる）。明細の自動集計は無く白紙から手入力する。
...
- 請求書ドラフトの自動生成は`src/server/billing/invoiceDraft.ts`に集約されている。取引先を指定すると`buildCounterpartyBillingSnapshot`が集計エンジンの結果からコース×ユニット×ドライバー別の明細行を自動で組み立てる。郵便局区分だけは集計エンジンを通らず`sales_log_entries`（手動売上ログ）を使う。ドライバーを指定すると`computeDriverAutoPayout`＋固定経費（有効期間で当月に絞る）＋臨時経費＋リース控除から受領請求書の明細を組む。
- **作成口は`POST /api/admin/invoices/from-source`の1本**（`{month, section, source:{type:"driver_payout"|"counterparty", ...}}`）。ペイメント画面・取引先画面・請求書一覧の作成ピッカーはすべてこれを呼び、サーバーで集計・採番・payload組み立てまで済ませて `id` を返す → `/admin/invoices/<id>/edit` へ遷移する。
  - 2026-08-18 まではこの3画面がそれぞれ別実装で、特にペイメント画面はクライアントで5本APIを叩いて明細を組み直していた。固定経費の有効期間を見ていない・消費税の既定が違うなど、**同じ月・同じドライバーでも作成導線によって中身が変わる**状態だったため統合した。
  - `GET /api/admin/invoices/draft`（編集画面が読む下書き）も同じ`invoiceDraft.ts`を使う。採番の重複回避は`src/server/billing/invoiceNumbering.ts`に一本化。
...
### 7-1. 税まわりの課題（2026-08-18 棚卸し・未着手）

請求書の税に関する操作が3つあり、役割が混同されやすい。**まず整理として**:

| | 実体 | 何を決めるか | 粒度 |
|---|---|---|---|
| 消費税チェック | `taxEnabled` + `taxRatePercent` | **課税するか**（OFF=税額0・免税事業者） | 請求書 |
| 表示 税抜/税込 | `displayBasis` | **帳票の見せ方**（外税で足すか内税で戻すか） | 請求書 |
| 行の「込」「抜」 | `priceBasis` | **入力した数字がどちらの意味か**（税のルールではない） | 行 |

**よくある誤解**: 行の「込/抜」は「この行だけ課税/非課税」ではない。表示列は常に統一される
（税込表示なら「抜」の行も `round(price×1.1)` に換算されて同じ列に並ぶ）。
`priceBasis` は換算の向きを決める**入力の注釈**でしかない。

存在する理由は**契約上のキリの良い数字を保つため**。「税込10,000円」の契約を税抜で持たせると
`floor(10000/1.1)=9,090` → 戻すと `9,999` で1円ズレる。税込のまま持てばズレない。

#### 問題点

1. **行の換算が税率10%固定**。`reward.ts` の `resolveRowPrice` は
   `exclusiveOf(price, basis)` を税率を渡さずに呼ぶため、`taxBasis.ts` の既定10%が使われる。
   消費税額そのもの（`computeInvoiceTotals`）は請求書の税率を使うので、
   **8%の請求書に「込」の行があると辻褄が合わない**。
   → ただし軽貨物運送で軽減税率は使わないため**優先度は低い**（2026-08-18 ユーザー確認）。
2. **印刷物に痕跡が残らない**。出力は統一列なので、後から「この行は税込入力だった」と分からない。
3. **押し間違いが約10%の金額差を無言で生む**。支払いに直結する書類で見えない失敗モード。
4. **生成側は一度も `inclusive` をセットしない**。下書きAPI・新規作成とも常に `exclusive`。
   「込」が付くのは手クリックのみ。
5. **1行ずつクリックするしかない**。`lineGrid.ts` の `EDIT_COLS` は
   `title/qty/unit/price` のみで `priceBasis` を含まないため、貼り付けもフィルも効かない。
6. **「非課税・不課税」の概念が無い**。立替金・預り金の行も必ず課税される。
   行単位で本当に必要な属性はこちらの可能性がある。

#### 検討した案（未採用・保留）

- 込/抜を**請求書単位の「入力基準」に格上げ**し、行チップを撤去する案。
  → **既存の請求書に `priceBasis="inclusive"` の行が多数存在する**（2026-08-18 ユーザー確認）ため、
  撤去にはデータ移行が要る。単独で進めず、税まわり全体の再設計とまとめて扱う。
- 当面の緩和策として「列ヘッダーから全行一括切替」「新規行が直前行の基準を引き継ぐ」。
  クリック数は減るが、上記1〜4は解決しない。
