...
- **候補の管理（2026-09-08）**: 地図（`/admin/map`）の拠点編集パネルに「日報の置き場所の候補に出す」（`CheckboxField`）。`PATCH /api/admin/map/places/:id` の `allowParking`、`POST` でも指定可（未指定は出す）。候補から外した拠点は設定の拠点一覧に「日報の候補外」と出る。客先・給油所など、車を置かない拠点を候補から外す用途。
- **モバイルの方針（2026-09-08 ユーザー指定）**: モバイルの日報では Web のような候補の選択はさせない。端末の位置情報と motion（停車の検知）から駐車位置を自動で特定し、同じ保存処理（`vehicle_positions` の申告行）へ流す。候補の選択 UI を移植しない。
- **未実装（設計どおり残す）**: 日報と駐車の同一トランザクション化、`in_use` 等の回答の永続化、シフトの「最後の駐車」表示、モバイル（自動特定）、QR、会社別の必須／任意設定、管理者の代理入力・訂正。
