...
→ 運営社オンボーディングは元々ハイタッチなので、**ログイン方式は導入相談時に顧客ごとに決める設定項目**にする（プロダクト全体で1つに決め込まない＝選択の余地を残す）。
...
5. **運営者の認証方式（§2-5）**: SSO / Passkey＋メール復旧 / メールOTP＋2FA のメニューをorg単位で選択。当面どれを既定にするか。ID＋パスワード単体は不採用。**一存で決めず、選択肢として残す（ステークホルダー協議）。**
6. **課金（§2-5）**: エンタープライズ価格＋導入相談必須。具体の価格テーブル・契約フローは未確定。
