...
#### 2026/09/17 認証強化（実装済み・本番未反映）

Passkey追加/削除に5分以内のSMS/Passkey確認を必須化。旧JWTは再確認へ誘導し、セッション更新で認証時刻を延長しない。鍵変更と本人通知をRPC166で同時保存し、SMS未確認時の最後の鍵の削除と同時削除を防ぐ。オンボーディングはPasskeyを主操作、登録できないときのみSMSで継続する。

ドライバーPIN UI/API・新規発行はローカルの廃止候補を準備したが、active DRIVER 17所属中11所属のSMS/Passkey移行が必要。SQL167と本番切替は移行後とする。運営のパスワード認証は維持。詳細・検証・公開順は [B-1〜B-8対応記録](design/security-b1-b8-2026-09.md)。
...
- **最終方針はPINの完全撤廃**（現行の driver_code＋6桁PIN、承認時の初期PIN発行を廃止）。6桁PINは弱く、OTP が復旧を兼ねるため不要。実装状況を方針と区別する: 招待・承認の新規PIN発行は停止済み。2026/09/17にWebのPasskey/SMS優先、PIN未発行者の変更欄非表示・APIでの新規作成拒否を実装した（未公開）。既存PINの認証・旧モバイル・管理者の手動作成時の発行は残り、移行後に閉じる。
...
③ Passkeyを登録する（2026/09/17変更: 半必須。登録を主操作とし、
   設定できない場合だけ説明を開いてSMSで続行。失敗は再試行できる）
...
- 2026/09/17のWeb改修では、登録の途中再開とSMSログイン後にも本人のPasskey有無を確認する。登録済みなら案内を重ねず、未登録なら共通の設定画面へ進む。SMSを選んだ人は `/me` で後から登録できる。APIで業務全体をPasskey必須にする変更ではない。詳細・検証範囲は [認証導線の変更記録](design/onboarding-passkey-2026-09.md)。
