...
B-1のDB公開権限対策、B-2の所属失効、B-7前半のPasskey再送防止は本番反映済み。B-3/B-4/B-5はローカル実装・検証済みで、本番DB163とWeb公開は未実施。後段の古い順序・未適用表記は当時の記録として扱う。

| 状態 | 作業 | 確認結果 |
|---|---|---|
| 実装済み・未公開 | B-3: 他社データの保護 | 他社IDの承認・却下・関連データ更新を拒否。正式な当日貸与車は維持。報告種別を会社別に分離。AST検査は48テーブルで指摘0、API回帰テスト通過 |
| 実装済み・未公開 | B-4: 非公開ファイル | 認可済み会社・本人・ドライバーの保存範囲を保存/署名/削除で強制。他社パス・不正パス・外部URLを拒否。17テスト通過 |
| 実装済み・未公開 | B-5: 請求書の住所表示 | br改行を保ち文字列として描画。タグの非実行を確認。会社設定の社印参照も編集/印刷で一致。通常・長文・特殊文字・多行をPDF実ファイルで確認 |

設計と適用時の注意: [他社データ・ファイル・住所の保護](design/tenant-files-invoices-2026-09.md)。全1,177テストと本番ビルドを通過。migration 163は開発DBで検証後に全てROLLBACKしており、適用済みではない。

| 次の順序 | 作業 | 完了の確認 |
|---|---|---|
| 1 | B-8: 依存関係の重大な指摘への対応 | Next.jsの修正版とsharp等の実行時依存を確認し、変更を絞って更新。画像処理・ログイン・帳票・ビルドを検証 |
| 2 | B-3/B-4/B-5の本番反映 | 切替手段を具体化して本番作業の確認を得る。旧Webの報告種別アクセスを止め、DB163→Webを連続して切替。会社分離・正規操作・非公開ファイルを確認 |
| 3 | UI・PDF・画像の監査と統一 | 請求書の改ページ/保存失敗/閲覧権限→QR単体/一括PDFの圧縮・画像待機→受託者控→シフトメモ/日別配車の補正と後片付け。実PNG/PDFを検証 |
| 4 | B-7後半: Passkeyの追加・削除時の本人確認 | 直近のSMS OTPまたはPasskeyによる確認を必須にする。初回登録・復旧・正規操作への影響と通知方法を整理 |
| 5 | モバイルの配布・認証準備 → B-6のPIN廃止 | EAS接続・本番HTTPS設定・ATS例外除去・SMS/Passkeyを整え、内部配布で確認してからPIN経路を閉じる。Apple登録・Team IDは最新状況を確認 |
| 6 | 駐車位置の自動取得・地図の仕上げ | 業務終了時に1回測位し、失敗時も日報保存を維持。実機のナンバー表示・生成と後片付けを検証 |

- 9月12日の`npm audit --omit=dev`は31パッケージ（critical 1/high 11/moderate 19）を報告。インストール済みNext.js 16.2.12は[AVIF画像最適化の公式アドバイザリ](https://github.com/vercel/next.js/security/advisories/GHSA-2xp9-vwfh-vxw4)の影響範囲で、告知の修正版は16.3.3。実際の到達条件と他の依存も調べ、B-8を繰り上げる。`npm audit fix --force`によるExpoの大幅な版戻しは行わない。
- UI監査を各改修の標準手順に追加済み。ソース上の不統一と、実ファイルで確認した問題は[UI・PDF・画像出力の監査](development/ui-export-audit.md)に分けて記録。45行請求書で余白が大きい4ページと合計欄の反復を確認しており、帳票全体を修正済みとはしない。
- 支払い・稼働期間の整理は運用の回答が揃い次第、変更前後の試算を作って進める。過去単価は当時の根拠を確定してから後埋めし、現在単価で一括補完しない。個人別の確認内容は公開ロードマップへ記載しない。
- 各画面の改修時に本番コードを使う隔離fixtureを追加・更新する。シフト自動作成はメモとの役割・正式シフトへの反映方法を整理してから着手する。
