...
B-1のDB公開権限対策は適用済み。B-2の所属失効とB-7前半のPasskey再送防止は実装・検証済みで、本番DBの161/162を適用しWeb公開を進めている。適用記録は [認証修正の適用記録](security/2026-09-12-auth-release.md)。以下を現在の順序とし、後段の古い順序・未適用表記は当時の記録として扱う。

| 順序 | 作業 | 完了の確認 |
|---|---|---|
| 1 | B-3: 他社データを扱えないことの検査・修正 | 既存の静的検査11件を分類し、id指定の一律除外を見直す。報告の承認・却下と車両詳細の関連行を調べ、他社IDでは404となり関連行も変わらないことをテストする |
| 2 | B-4: 非公開ファイルの署名URL | 保存済み添付と会社の所有関係を確認し、他社パスから閲覧URLを作れないことをテストする |
| 3 | B-5: 請求書の住所表示 | 住所をHTMLとして実行せず文字列として表示する。既存の改行・帳票・PDFを維持し、悪意のあるタグが実行されないことをプレビューとテストで確認する。CSP強化は帳票出力への影響を確認して分けて扱う |
| 4 | B-7後半: Passkeyの追加・削除時の本人確認 | 直近のSMS OTPまたはPasskeyによる確認を必須にする。初回登録・復旧・本人の正規操作を妨げないことを確認し、通知方法も整理する |
| 5 | モバイルの配布・認証準備 → B-6のPIN廃止 | EAS接続・本番HTTPS設定・ATS例外除去・SMS/Passkeyを整え、内部配布でログインを確認してからPIN経路を閉じる。コード上のbundleIdは `jp.hakotora.app` で確定済み。Apple登録・Team IDは着手時に最新状況を確認する |
| 6 | 駐車位置の自動取得・地図の仕上げ | モバイルの業務終了時に1回測位し、位置取得が失敗しても日報を保存する。地図の実機でのナンバー表示・生成と後片付けを確認する |

- B-8の依存関係監査は1と合わせて実施し、該当する重大な問題があれば修正順を繰り上げる。現在の版が影響を受けるかは監査時に確認する。
