...
- [ ] **A3: シフト画面のUI改善（洗い出し 2026-07-20 実施）** — Quick win 4件は同日対応済み:
      ...
      ③便指定休バッジ拡大（9→11px） ④セル編集モーダルの車両リストから貸出中トグル（右上「貸」）。
      ...
      コピーしない）＋**空き枠バッジ**（日付ヘッダーに「空きN」アンバー表示、ホバーでコース別内訳）。
      残候補（要否は運用しながら判断）: コース軸ビュー（行=コースの切替。定員割れの俯瞰と
      コース起点の割当。ユーザーへ説明済み・判断待ち）/ セルの密度切替（車両非表示）/
      セルへの時間表示 / 手動リフレッシュ（多人数同時編集）。
