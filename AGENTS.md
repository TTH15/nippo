...
## UI監査と出力処理の一貫性（2026-09-12 ユーザー指定）

- 画面改修の標準手順にUI監査を含める。見た目・操作・状態・エラーをPC/スマホの隔離プレビューで確認し、未検証範囲を残す。
- PDF・画像を扱う場合は共通実装の使用、圧縮、読み込み待機、出力の一致、失敗復帰、後片付けも監査する。生成方式の違いだけを不具合とせず、用途上の理由を確認する。
- 継続する手順と優先順位は `docs/development/ui-export-audit.md` を参照する。
