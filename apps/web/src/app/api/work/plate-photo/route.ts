...
// POST: QRが読めない時の退避ルート（vehicle-session-flow §8.5）で撮るナンバープレート写真を
// 非公開 Storage にアップロードし、保存パスを返す。バケットは点検写真と共用（同じ「車両セッションの
// 証跡写真」という性質のため。migration 103）。返った path を check-in/out の platePhotoPath に渡す。
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    console.error("[work/plate-photo]", err);
