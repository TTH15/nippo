...
function measurePlate(vehicle: Vehicle) {
  const host = document.createElement("div");
  host.setAttribute("aria-hidden", "true"); host.inert = true;
  Object.assign(host.style, { position: "fixed", left: "-10000px", top: "0", width: `${PLATE_WIDTH}px`, pointerEvents: "none" });
  document.body.append(host);
  const root = createRoot(host);
  try {
    flushSync(() => root.render(createElement(DemoPlate, { vehicle, className: "w-full" })));
...
export async function renderPlateImage(vehicle: Vehicle): Promise<PlateImage> {
  // Reactのeffect中から呼ばれても、別rootの同期レイアウトを安全に取得する。
  await document.fonts.ready;
  const plate = measurePlate(vehicle);
