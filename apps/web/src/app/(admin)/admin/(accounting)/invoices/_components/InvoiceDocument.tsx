...
// Vanilla 版（public/invoice）の新デザインを React + Tailwind で再現したもの。
// 計算は @repo/core の computeInvoiceTotals に集約（支払いに直結するため単一の真実）。

/** 帳票1行（税抜単価モデル）。InvoiceRow を表示用に正規化したもの。 */
...
export type InvoiceDocData = {
  ...
  fromTel?: string;
  fromReg?: string;
  showStamp: boolean; // ACE CREATION のときのみ印鑑表示
  // メタ
  period: string;
  invoiceNo: string;
  // 金額
...
const BRAND = "#1f3b66"; // サマリー・タイトルの濃紺
const BILL = "#1f5fa8"; // 請求テーブルの青
const DEDUCT = "#1f7a5f"; // お支払いテーブルの緑

const jpy = (n: number) => Number(n || 0).toLocaleString("ja-JP");
const price = (n: number) =>
  Number(n || 0).toLocaleString("ja-JP", { maximumFractionDigits: 2 });

/** 明細テーブル（請求分 / お支払い分）。color で色テーマを切替。 */
        ...
        {lines.length === 0 ? (
          ...
          lines.map((ln, i) => (
            <tr key={i}>
              ...
              <td className="py-[2.5px] px-2 text-right bg-white" style={{ border: `1px solid ${color}` }}>{ln.price ? price(ln.price) : ""}</td>
...
export function InvoiceDocument({
  ...
  // サマリー行（▲はマイナス項目の金額前に表示）
  const summaryRows: { label: string; value: number; minus?: boolean }[] = [
    { label: "請求額 税込合計", value: totals.billGross },
    { label: "お支払い分 税込合計", value: totals.deductGross, minus: true },
    { label: "借入返済", value: data.loanRepay, minus: true },
    { label: "追加外注支払い（税込）", value: data.extraOutsourcing },
  ];
    ...
    <div className={cn("bg-slate-100 overflow-auto py-6", className)}>
      ...
      >
        ...
        <div
          ...
          style={{ color: BRAND, borderBottom: `3px solid ${BRAND}` }}
        >
          御 請 求 書
        </div>
        ...
        >
          <div className="text-[12.5px] font-semibold">ご請求金額</div>
          <div className="text-[18px] font-bold text-right" style={{ color: BRAND }}>
            ¥{jpy(totals.total)}（税込）
          </div>
        </div>
        ...
        <table className="w-full border-collapse text-[12px] mt-2 mb-[26px]">
          <thead>
            <tr>
              ...
              <th className="border-0 bg-transparent text-right font-semibold text-[11px] py-[1px] px-[9px]" style={{ color: BRAND }}>（円）</th>
            </tr>
          </thead>
          <tbody>
              ...
              <tr key={i}>
                <td
                  ...
                  style={{
                    ...
                    borderTop: i === 0 ? `2.5px solid ${BRAND}` : undefined,
                    borderLeft: `2.5px solid ${BRAND}`,
                ...
                <td
                  ...
                  style={{
                    border: `1px solid ${BRAND}`,
                    borderTop: i === 0 ? `2.5px solid ${BRAND}` : undefined,
                    borderRight: `2.5px solid ${BRAND}`,
                  }}
                >
                  {row.minus ? "▲" : ""}{jpy(row.value)}
                </td>
              </tr>
            ))}
            <tr className="text-white font-bold text-[15px]">
              <td className="py-[3.5px] px-[9px] text-left" style={{ border: `1px solid ${BRAND}`, borderBottom: `2.5px solid ${BRAND}`, borderLeft: `2.5px solid ${BRAND}`, backgroundColor: BRAND }}>差引き請求額</td>
              <td className="py-[3.5px] px-[9px] text-right" style={{ border: `1px solid ${BRAND}`, borderBottom: `2.5px solid ${BRAND}`, borderRight: `2.5px solid ${BRAND}`, backgroundColor: BRAND }}>{jpy(totals.total)}</td>
            </tr>
          </tbody>
        </table>
        ...
        <LineItemsTable
          ...
          subtotalLabel="お支払い分小計（税抜）"
          taxLabel={data.taxEnabled ? `消費税額（お支払い分 ${ratePct}%）` : ""}
        />

        {/* 振込先 */}
        <div className="mt-[14px] pt-2 text-[11.5px] leading-[1.4]" style={{ borderTop: `2.5px solid ${BRAND}` }}>
