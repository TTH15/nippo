...
const { data } = await supabase.from('invoice_documents').select('id, invoice_no, client_name, amount, payload').in('id', ids);
for (const row of data) {
  console.log(`\n########## ${row.client_name} ${row.invoice_no} amount=${row.amount} ##########`);
  console.log('main:', JSON.stringify(row.payload.tableData.main));
  console.log('deduct:', JSON.stringify(row.payload.tableData.deduct));
}
