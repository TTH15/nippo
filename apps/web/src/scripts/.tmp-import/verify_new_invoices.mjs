...
const { data, error } = await supabase.from('invoice_documents').select('id, invoice_no, status, amount, driver_id, client_name, payload').in('id', ids);
if (error) { console.error(error); process.exit(1); }
for (const row of data) {
  const main = row.payload?.tableData?.main ?? [];
  const deduct = row.payload?.tableData?.deduct ?? [];
  const hasBadLine = [...main, ...deduct].some(l => !l.title || l.qty == null || l.price == null);
  console.log(`${row.client_name} ${row.invoice_no} status=${row.status} amount=${row.amount} mainLines=${main.length} deductLines=${deduct.length} 異常行=${hasBadLine}`);
}
