...
const { data } = await supabase.from('invoice_documents').select('client_name, invoice_no, status, amount').in('id', ids).order('client_name');
data.forEach(r => console.log(`${r.client_name}\t${r.invoice_no}\t${r.status}\t${r.amount.toLocaleString()}円`));
