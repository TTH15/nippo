...
export async function GET(req: NextRequest) {
  ...
  const [config, { data: drivers }, { data: carriers }, { data: units }, { data: fields }] =
    await Promise.all([
  ...
  return NextResponse.json({ config, drivers: drivers ?? [], carriers: carrierTree });
}
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: "保存に失敗しました（migration 061 未適用の可能性）" },
      { status: 500 },
    );
  }
